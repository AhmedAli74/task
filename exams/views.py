...
import untitled3
from untitled3 import evaluate_answers,preprocess_text
...
def score_details(request, score_id, category_slug, subject_slug):
    grade = Category.objects.get(slug=category_slug)
    subject = Subject.objects.get(category=grade, slugy=subject_slug) 
    questions = Question.objects.get(subjects=subject,id=score_id)
    answer=Answrer.objects.filter(question=questions)
    score_written=evaluate_answers(request,answer)
    ...
    #     submitted_answers = request.POST.getlist('answer')
    #     for answer_text in submitted_answers:
    #         nlp_answer = Answrer(answer_text=answer_text)
    #         nlp_answer.save()

    #     questions = Question.objects.filter(category=category_slug, subject=subject_slug, id=score_id)
